...
/** The text cursor glyph. Single source so every input agrees on it. */
export const CURSOR = "▌";
...
export function Input({ id, value, onChange, onSubmit, placeholder, mask, disabled = false, autoFocus = true }: InputProps) {
  const { isFocused } = useFocus({ id, isActive: !disabled, autoFocus });
  useInput((input, key) => {
  ...
  return (
    <Box borderStyle={isFocused ? "double" : "single"} paddingX={1}>
      {/* Cursor leads the placeholder when empty, so a focused empty field
          never looks identical to an unfocused one. Rendered as its own Text
          so it stays bright while the placeholder behind it stays dim. */}
      {isFocused && !value ? <InkText>{CURSOR}</InkText> : null}
      <InkText dimColor={!value}>{shown}</InkText>
      {isFocused && value ? <InkText>{CURSOR}</InkText> : null}
    </Box>
  );
}
...
export function MultilineInput({ id, value, onChange, onSubmit, cursor = { offset: value.length }, onCursorChange, selection, onSelectionChange, placeholder, minRows = 3, disabled = false, autoFocus = true }: MultilineInputProps) {
  const { isFocused } = useFocus({ id, isActive: !disabled, autoFocus }); const { capabilities } = useFancyTui();
  ...
  const offset = Math.max(0, Math.min(value.length, cursor.offset));
  const rendered = value ? `${value.slice(0, offset)}${isFocused ? CURSOR : ""}${value.slice(offset)}` : placeholder ?? "";
  return (
    <Box borderStyle={isFocused ? "double" : "single"} paddingX={1} minHeight={minRows}>
      {/* The empty case previously short-circuited to the placeholder and
          skipped the cursor entirely, so a focused empty composer was
          indistinguishable from an unfocused one apart from the border. */}
      {isFocused && !value ? <InkText>{CURSOR}</InkText> : null}
      <InkText dimColor={!value}>{rendered}</InkText>
    </Box>
  );
}
export const Textarea = MultilineInput;

function ChoiceList({ id, options, selected, onToggle, multiple = false, disabled = false, autoFocus = true, marker }: { id: string; options: Option[]; selected: string[]; onToggle: (id: string) => void; multiple?: boolean; disabled?: boolean; autoFocus?: boolean; marker?: (on: boolean) => string }) {
  const { isFocused } = useFocus({ id, isActive: !disabled, autoFocus });
...
export function RadioGroup({ id, value, onChange, options, disabled, autoFocus }: InteractiveProps & { value?: string; onChange: (value: string) => void; options: Option[] }) { return <ChoiceList id={id} options={options} selected={value ? [value] : []} onToggle={onChange} disabled={disabled} autoFocus={autoFocus} />; }
export function Switch({ id, checked, onChange, label, disabled, autoFocus }: InteractiveProps & { checked: boolean; onChange: (value: boolean) => void; label: string }) { return <Checkbox id={id} checked={checked} onChange={onChange} label={`${label} ${checked ? "ON" : "OFF"}`} disabled={disabled} autoFocus={autoFocus} />; }
export function MultiSwitch({ id, value, onChange, options, disabled, autoFocus }: InteractiveProps & { value: string; onChange: (value: string) => void; options: Option[] }) { return <RadioGroup id={id} value={value} onChange={onChange} options={options} disabled={disabled} autoFocus={autoFocus} />; }
...
export function Slider({ id, value, onChange, min = 0, max = 100, step = 1, disabled, autoFocus = true }: InteractiveProps & { value: number; onChange: (value: number) => void; min?: number; max?: number; step?: number }) {
  const { isFocused } = useFocus({ id, isActive: !disabled, autoFocus }); useInput((_i, key) => { if (!isFocused) return; if (key.leftArrow) onChange(Math.max(min, value - step)); if (key.rightArrow) onChange(Math.min(max, value + step)); });
